...
const Comments = ({
  ...
  useEffect(() => {
    if (state.success) {
      // Reset the form or perform any other action on success
    ...
    <div className="">
      {user && (
        <form action={formAction} className="flex items-center justify-between gap-4 p-4 ">
          ...
          <button
            disabled={isPending}
            className="disabled:cursor-not-allowed disabled:bg-slate-200 py-2 px-4 font-bold bg-white text-black rounded-full"
          >
